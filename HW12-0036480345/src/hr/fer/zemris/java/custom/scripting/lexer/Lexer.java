...
public class Lexer {
	...
	/**
	 * A {@link List} which will be populated with the tokenised text.
	 */
	private List<Token> tokenised;
	...
	private static final List<Character> escapesText;
	static {
		escapesText = new ArrayList<>();
		escapesText.add('{');
		escapesText.add('\\');
	}

	/** A list of allowed escapes in {@code Strings}. */
	private static final List<Character> escapesSpecial;
	static {
		escapesSpecial = new ArrayList<>();
	...
	/** A list of allowed escapes in {@code Strings}. */
	private static final List<Character> escapesString;
	static {
		escapesString = new ArrayList<>();
		escapesString.add('"');
		escapesString.add('\\');
	}
	 ...
	 * locally in a {@link List}. Also used for the FUNCTION {@link TokenType}s,
	 * since both of them are using the same syntax.
	 * 
	 * @param startIndex
	 ...
	 * locally in a {@link List}. Saves both integer and double
	 * {@link TokenType}s.
	 * 
	 * @param startIndex
	 *            represents the index in the local {@link Character} array
	 ...
	 * locally in a {@link List}. Checks for valid escaping using the
	 * {@link #checkEscape} method.
	 * 
	 * @param startIndex
	 *            represents the index in the local {@link Character} array
	 *            where the token data begins.
	 ...
	 * 
	 * @param escapes
	 *            an {@link List}, in which the legal escape sequences are
	 *            stored.
	 * @return returns true if the escape sequence
	 */
	private boolean checkEscape(List<Character> escapes) {
		if ((data[currentIndex]) == '\\') {
			if (currentIndex + 1 < data.length
				...
				if (currentIndex < data.length && escapesSpecial.contains(data[currentIndex])) {
					int temp = (data[currentIndex] == 'r') ? 13 : 10;
